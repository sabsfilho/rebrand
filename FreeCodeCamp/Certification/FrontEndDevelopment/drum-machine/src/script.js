...
class Btns extends React.Component {
  ...
  render(){
    ...
    KEYS.forEach(key => btns.push(
      <button id={'btn-'+key} class="drum-pad" onClick={this.props.onPressed}>
        <audio id={key} class="clip" src={acts[hackMP3Forbidden(key)].src}></audio>{key}
      </button>
    ));
...
class Drum extends React.Component {
  ...
  render(){
    return (
      <div>
        <Btns state={this.state} onPressed={this.onPressed} />
